...
frame_builder[C.FRAME_TYPE.TX_REQUEST_64_LOWPAN] = function(frame, builder) {
  builder.appendUInt8(C.FRAME_TYPE.TX_REQUEST_64); // IT IS A TX REQUEST 64 
  builder.appendUInt8(this.getFrameId(frame));
  builder.appendString(frame.destination64 || C.UNKNOWN_64, 'hex'); 
  builder.appendUInt8(C.sixlowpan_options); // options is 2
  builder.appendUInt16BE(C.sixlowpan_first_part); // 6LOWPAN  
  builder.appendUInt16BE(C.sixlowpan_second_part); //Common header  
  builder.appendUInt16BE(frame.sendPort); // SEND PORT
  builder.appendUInt16BE(frame.dstPort); // DESTINATION PORT
  builder.appendUInt16BE((chk.chksum(frame))); // UDP CHECKSUM 
  appendData(frame.data, builder);
};
